...
class MINIST(BaseDataset):
    ...
    def get_split_datasets(args):
        ...
        """
        logger.info("use minist")
        root = PRO_DIR+'/minist'

        general_data_transform = transforms.Compose(
            [transforms.ToTensor(),transforms.Normalize(MEAN['mnist'], STD['mnist'])]
        )
        train_part = torchvision.datasets.MNIST(root, True, download=True,transform=general_data_transform)
        test_part = torchvision.datasets.MNIST(root, False, download=True,transform=general_data_transform)
