...
class CIFAR10(BaseDataset):
    ...
    def get_split_datasets(args):
        ...
        """
        logger.info("use cifar10")
        root = PRO_DIR+'/cifar10'

        general_data_transform = transforms.Compose(
            [transforms.ToTensor(),transforms.Normalize(MEAN['cifar10'], STD['cifar10'])]
        )
        train_part = torchvision.datasets.CIFAR10(root, True, download=True,transform=general_data_transform)
        test_part = torchvision.datasets.CIFAR10(root, False, download=True,transform=general_data_transform)
