...
class FMINIST(BaseDataset):
    ...
    def get_split_datasets(args):
        ...
        """
        logger.info("use fminist")
        root = PRO_DIR+'/fminist'

        general_data_transform = transforms.Compose(
            [transforms.ToTensor(),transforms.Normalize(MEAN['mnist'], STD['mnist'])]
        )
        train_part = torchvision.datasets.FashionMNIST(root, True, download=True,transform=general_data_transform)
        test_part = torchvision.datasets.FashionMNIST(root, False, download=True,transform=general_data_transform)
        ...
        logger.info("cifar10 load success!")
        return ret